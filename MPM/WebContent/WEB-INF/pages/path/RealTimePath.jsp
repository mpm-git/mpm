...
// 						},
				...
				timerTask.stopTaskOther(person.empNO);
				timerTask.setTask(person.empNO,function(){
					$.ajax({ 
						url: r_person_resltime_location, 
						data:{empNO:person.empNO},
						success: function(data){
							if(data&&data.nTime)
							{
								if (_element.param.text.time != data.nTime) {
									_element.moveTo({
										x : data.nX,
										y : data.nY
									}, {
								...
								_element.param.text.time = data.nTime;
								console.info(data);
							}
						}
					});
				});
				timerTask.runTask(person.empNO, 2000);
